...
def singly_login_url(context, service):
    ...
    auth_url = "https://api.singly.com/oauth/authenticate?client_id=%s&redirect_uri=%s&service=%s" % (
        getattr(settings, 'SINGLY_CLIENT_ID', None),
        redirect_uri,
        service
    )
    if hasattr(context['user'], 'singly'):
        auth_url = "%s&access_token=%s" % (auth_url, context['user'].singly.access_token)
    return auth_url
